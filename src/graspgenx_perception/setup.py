...
# 파일을 옮기면 scripts/test_*.py 의 import 가 깨지고 git 이력이 끊긴다 — 경로는
# package_name 으로 통일하되(ros2 run graspgenx_perception <script>) 소스 위치는 건드리지 않는
# 절충이다. colcon 은 build/graspgenx_perception/ 를 cwd 로 setup.py 를 돌린다. setup.py
# 자신의 위치에서 거슬러
# 올라가야 cwd 에 무관하게 맞는다.
