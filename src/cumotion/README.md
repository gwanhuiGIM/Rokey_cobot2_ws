<!-- meta
updated: 2026-08-08
status:  live (2026-08-08)
         ✅ **장애물 회피 최초 성공** — goal 경계 조건(⭐-3c). 실행 중 회피는 아직 안 됨
         ✅ 대조군 실증 — 예제 방식은 실행 중 회피 안 됨을 실기로 확인(⭐절). 새 파일 2개
         ✅ pose(xyz) 목표 동작 확인. 관절/xyz 는 파일 상단 GOAL_MODE 블록에서 고른다(⭐-3)
         🎯 경유점(waypoints) 추가 — 회피 반응 8.2→4.4초 (⭐-3b, **실기 미검증**)
         🔴 **그리퍼 SRDF 자기충돌 4쌍 누락**(⭐-2) — 계획이 조용히 버려진다.
            INVALID_MOTION_PLAN(-2) 의 진짜 원인일 공산이 크다
         🔴 실험군(reactive_replan)은 _same_path 시간정렬 버그로 아직 못 움직인다(⭐-4 1번)
next:    waypoints:=2 실기 검증(⭐-3b) → 그리퍼 열고 재시험(⭐-2) → _same_path(⭐-4 1번)
