...
class OnRobotRGNode(Node):
    ...
    def initParams(self):
        ...
        self.grip_pub = self.create_publisher(Bool, '/onrobot/grip_detected', 3)
    ...
    def getStatus(self):
        ...
        # grip은 별도 레지스터가 아니라 gSTA 상태워드(register 268)의 bit1.
        # status['grip'](register 269)은 grip이 아니라 항상 non-zero → 오검출.
        # status['busy']에 gSTA 전체워드가 담겨있음(_baseOnRobotRG.py: gsta=status[10]).
        gsta = int(self.status['busy'])
