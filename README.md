...
| **마이크 노드(`get_keyword.py`)의 LLM 프롬프트 어휘가 `config/objects.yaml`과 무관하게 하드코딩돼 있었다** | 프롬프트의 `<도구 리스트>`가 `hammer, screwdriver, wrench, pos1, pos2, pos3`였다 — 이 ws의 YOLO(`detect: bottle, cup, spoon, banana, apple, orange, mouse`)엔 그런 클래스가 없어, 음성으로 뭐라 말하든 브리지가 "해당하는 물체가 없다"로만 실패했다. `vla_command_node`는 이미 `allowed_classes`로 같은 검사를 하고 있었는데 마이크 경로만 빠져 있었다 | `get_keyword.py`가 기동 시 `config/objects.yaml`의 `detect`를 읽어 프롬프트에 그대로 넣도록 고침(`COBOT2_OBJECTS` 환경변수로 경로 덮어쓰기 가능, 다른 노드들과 같은 규칙) — 상세는 [`src/PACKAGES.md#voice_processing`](src/PACKAGES.md#voice_processing) |
