...
- 자세는 회전을 충분히 섞어야 한다. 평행이동만 하면 캘리브레이션이 수렴하지 않는다.
  촬영 각도 체크리스트 (근거: eye2hand_calibration.py의 report_pose_quality 임계값, 2026-08-10):
    · 개별 기울기: 보드 법선-카메라 광축 각도 20~50° (45° 근방 이상적, Zhang 1998).
      0°(정면)는 무한원면에서 같은 제약만 반복해 퇴화, 50° 넘으면 foreshortening으로
      코너검출 자체가 무너진다(위 32장/5검출 사고가 이 각도 문제였을 가능성).
    · 기울이는 축: 장마다 다르게 — 위/옆/대각선을 섞는다. 한 축(예: J4)으로만 갸웃거리면
      법선이 한 평면을 쓸고 지나가 내부적으로 rank가 부족해진다.
    · 거리: 가까운 장·먼 장 섞기 (최대/최소 비 1.3배 이상 목표).
    · 연속 두 장 사이 상대회전: 30° 이상 (logR()이 0/0에 가까워지는 것을 피함).
    · 연속 두 장 사이 회전축: 직전 장과 겹치지 않게 (Calibrate()의 외적이 0벡터가 안 되도록).
    · 목표 장 수: 최소 8장 생존, 코너검출 실패를 감안해 20장+ 촬영 권장.
    촬영 후 `--pose-quality`로 이 조건들을 실제로 만족했는지 먼저 확인할 것 (아래 참고).
- 🔴 **수집 전에 반드시 이 명령으로 카메라를 다시 띄운다. 안 하면 캘리브가 통째로 버려진다:**
    ros2 launch m0609_rg2_bringup camera.launch.py color_profile:=1280x720x15
  이 스크립트는 해상도를 지정하지 않고 구독만 하므로, 런치 **기본값 424x240** 그대로 수집하면
  10x7 코너가 거의 안 잡힌다. 2026-08-09에 실제로 그렇게 32장을 찍었고 **5장만 검출**되어
  병진잔차 41.1 mm(불합격)가 나왔다. 직전 1280x720 수집분은 18/26장 검출 · 6.0 mm였다.
  → 수집을 시작하기 전에 `cv2.imshow` 창 크기나 `ros2 topic echo --field height`로 확인할 것.
- ⚠️ **`data/`는 매번 덮어쓰인다.** 이 스크립트는 항상 `data/`에 쓴다. 이전 수집분을 남기려면
  **수집 시작 전에** `mv data data<N>` 해 둔다 (`eye2hand_calibration.py data<N>`으로 재계산 가능).
- 수집 후 `python3 eye2hand_calibration.py`가 RMS 재투영오차·AX=XB 잔차·LOO 안정성을
  찍고 `calib_report.json`으로 남긴다. **그 숫자를 보고 재수집 여부를 정한다.**
  결과 행렬만 보고 넘어가지 말 것 — 특히 `n_usable_images`가 표본 수다(수집 자세 수가 아니다).
"""
